...
class WalletPaymentHistoryApi extends api.WalletApi {
  ...
  async getHistory(parameters) {
    return await this.http.get(
      url.url`payment-history/v2/persons/${this.walletId}/payments`(parameters)
    );
  }
  /**
   * ## Статистика платежей
  ...
  async getTotal(parameters) {
    return await this.http.get(
      url.url`payment-history/v2/persons/${this.walletId}/payments/total`(parameters)
    );
  }
  /**
  ...
  async getTransaction(transactionId, type) {
    return await this.http.get(
      url.url`payment-history/v2/transactions/${transactionId}`({ type })
    );
  }
  /**
   * ### Файл квитанции
  ...
  async getTransactionCheque(
    ...
    return await this.http.request({
      url: url.url`payment-history/v1/transactions/${transactionId}/cheque/file`({
        type,
        format
      }),
      method: "GET",
  ...
  async sendTransactionCheque(transactionId, type, email) {
    await this.http.post(
      url.url`payment-history/v2/transactions/${transactionId}/cheque/send`({
        type
      }),
      { email }
    );
