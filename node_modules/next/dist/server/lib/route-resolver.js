...
async function makeResolver(dir, nextConfig, middleware, serverAddr) {
    ...
    router.compiledRoutes = router.compiledRoutes.filter((route)=>{
        var _route_name;
        return route.type === "rewrite" || route.type === "redirect" || route.type === "header" || route.name === "catchall route" || route.name === "middleware catchall" || route.name === `_next/${_constants.CLIENT_STATIC_FILES_PATH}/${buildId}/${_constants.DEV_MIDDLEWARE_MANIFEST}` || ((_route_name = route.name) == null ? void 0 : _route_name.includes("check"));
    });
